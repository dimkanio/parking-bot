...
from src.config import TOKEN, PARKING_CHAT_ID, LOGLEVEL
from src.messages import MESSAGES, TgAddresses
import src.keyboards as kb
from src.dbhelper import DBHelper
from src.stateflow import TestStates
from src.validator import Valid
from src.parkmap import ParkMap
...
log_level = levels.get(str(LOGLEVEL).upper())

try:
    logging.basicConfig(format=u'%(filename)+13s [ LINE:%(lineno)-4s] %(levelname)-8s [%(asctime)s] %(message)s', level=log_level)
except ValueError:
...
async def prepare_mm_map_for_message():
    ...
    message += "\nСсылка действительна только сегодня."  
    return message
